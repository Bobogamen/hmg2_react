...
const PATH_COLORS = [
  ...
  { path: "/fund", color: "text-bg-secondary bg-opacity-75" },
  { path: "/repair", color: "text-bg-success bg-opacity-75" },
  { path: "/statistics", color: "text-bg-info bg-opacity-75" },
  { path: "/cashier", color: "text-bg-dark bg-opacity-50" },
];

const getSectionColor = (pathname) =>
  PATH_COLORS.find((item) => pathname.startsWith(item.path))?.color ??
  "text-bg-light";
            ...
            <React.Fragment key={i}>
              ...
              >
                {crumb.path && !isLast ? (
                  ...
                  >
                    <span
                      className={useCustomColor ? "breadcrumb-item-text" : null}
                    >
                      {crumb.label}
                    </span>
                  </Link>
